...
export function RivalsPanel() {
            ...
            <div className="h-14 px-4 flex items-center justify-between border-b border-border">
                ...
                <div className="flex items-center gap-1">
                    ...
                    <button
                        ...
                        title="Close panel"
                    >
                        {/* On mobile show X, on desktop show panel close icon */}
                        <X size={14} className="md:hidden" />
                        <PanelRightClose size={14} className="hidden md:block" />
                    </button>
                </div>
            </div>
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-0.5 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                        <button
                                            ...
                                            className="p-1.5 rounded text-muted-foreground hover:text-foreground transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                        ...
                                        <button
                                            ...
                                            className="p-1.5 rounded text-muted-foreground hover:text-danger transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                            title="Remove rival"
                                        >
                                            {removingRival === rival ? (
                                                <Loader2 size={13} className="animate-spin" />
    ...
    return (
        <>
            ...
            <div
                className={cn(
                    "hidden md:flex h-full bg-card flex-col shrink-0 transition-all duration-300 ease-in-out overflow-hidden border-border",
            ...
            {!rightPanelOpen && (
                <button
                    ...
                    className="hidden md:block fixed right-0 top-3 z-30 p-2 bg-card border border-border border-r-0 rounded-l-md text-muted-foreground hover:text-foreground transition-colors"
            ...
            {rightPanelOpen && (
                <>
                    <div
                        ...
                        onClick={() => setRightPanelOpen(false)}
                    />
                    <div className="md:hidden fixed inset-y-0 right-0 z-50 w-[85%] max-w-sm bg-card border-l border-border shadow-2xl animate-slide-in-right">
                        {panelContent}
                    </div>
                </>
            )}
            ...
            {!rightPanelOpen && (
                <button
                    onClick={() => setRightPanelOpen(true)}
                    className="md:hidden fixed right-3 bottom-[4.5rem] z-30 p-3 bg-card border border-border rounded-full text-muted-foreground hover:text-foreground transition-colors shadow-lg"
                    title="Open rivals panel"
                >
                    <Users size={18} />
                </button>
